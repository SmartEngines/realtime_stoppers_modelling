...
class TreapBasedSequenceStructure:
    ...
    def get_modelling_sum_gld(Y, n):
        '''
        Returns approximation of the sum of distances (unnormalized) required for next combined result modelling
        '''
        ret = 0.0
        for cell in Y:
            for c in cell.keys():
...
class AlignmentWithEstimation:
    ...
    def get_modelling_sum_gld(self):
        '''
         Returns approximation of the sum of distances (unnormalized) required for next combined result modelling
        '''
        return self.SequenceStructure.get_modelling_sum_gld(self.Y, self.base_samples)